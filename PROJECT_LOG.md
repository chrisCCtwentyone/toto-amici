...
- 📋 **Comando admin per correzioni manuali**: progettato in dettaglio (due flussi: correzione pronostico singolo, inserimento risultato manuale quando l'API è bloccata), con validazione stretta lato bot per il campo testo libero (regex `^\d{1,2}-\d{1,2}$` per i risultati, per escludere a priori anche input tipo "formula injection" su Sheets con `valueInputOption=USER_ENTERED`, che interpreterebbe un valore che inizia con `=`/`+`/`-`/`@` come formula). **Non ancora implementato** — in attesa di conferma finale dell'utente.
