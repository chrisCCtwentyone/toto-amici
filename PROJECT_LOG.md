...
1. **«L'ha già caricata lui dieci minuti fa»**: già bloccato dal commit `16f856d` (blocco anti-doppione, arrivato sopra il commit multi-admin nella stessa giornata). `scrivi_su_sheets_con_regole` rilegge `Giocate!A:B` prima di scrivere e solleva `SchedinaGiaPresente`. Nato per il doppio caricamento della stessa persona, copre gratis anche due admin diversi.
2. **Lo stesso istante.** Fra quella rilettura e la scrittura passa circa un secondo: due «Salva» premuti insieme trovano **entrambi** il posto libero e scrivono entrambi — il controllo non serve proprio nel caso che doveva coprire. Chiuso con `lock_salvataggio_schedina()`, un `asyncio.Lock` attorno a rilettura + controllo + scrittura. Il bot è un processo solo su Render, quindi serializzare qui basta: un lock distribuito sarebbe sproporzionato. Non copre una modifica fatta a mano sul foglio nello stesso istante, e non è il caso in questione.
3. **Nessuno dei due ha ancora salvato**, perché stanno entrambi caricando le foto — che è **letteralmente lo scenario chiesto dall'utente**, ed è il più largo dei tre: su Sheets non c'è ancora niente da trovare, quindi nessun controllo può accorgersene. Se ne accorgevano alla fine, e chi perdeva aveva buttato via la lettura IA e l'attesa. Coperto da `LAVORAZIONI_IN_CORSO`, un registro in memoria `(giornata, giocatore) -> (admin, nome, inizio)`: quando l'altro admin sceglie la stessa coppia, la schermata di conferma lo avvisa **prima** di chiamare Gemini.

**Il registro è un avviso, non un lucchetto**, e la scelta è deliberata: l'altro admin potrebbe abbandonare a metà, e bloccare una schedina per un lavoro che non arriverà mai sarebbe peggio del problema. Una lavorazione scade dopo 15 minuti e viene tolta dalla memoria; si libera da sola a fine caricamento o su `/cancel`, e `libera_lavorazione()` toglie **solo la propria** — annullare il proprio caricamento non deve sbloccare quello di un altro.

**Controllo anticipato anche su Sheets** (`schedina_gia_nel_foglio()`): la stessa lettura `A:B` fatta *prima* della chiamata IA invece che dopo. Scoprire il doppione alla fine significa aver speso una chiamata a Gemini e una ventina di secondi. **Se il foglio non risponde si procede lo stesso**: questo è un anticipo, la guardia vera resta quella al salvataggio, e un errore di lettura qui deve al massimo far perdere il vantaggio — mai impedire di caricare una schedina.

**Test: 478 totali.** Nuovo `tests/test_lavoro_parallelo.py` (17 test). Quello centrale lancia due `esegui_salvataggio_ia` con `asyncio.gather` su un foglio finto che riproduce la finestra vera fra controllo e scrittura: **verificato togliendo il lock**, senza il quale la schedina viene scritta due volte e il test fallisce. Uniformato anche il messaggio «Schedina già presente», che in una copia era scritto senza accenti.
