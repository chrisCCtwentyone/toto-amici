...
- [ ] **Multi-admin**: possibilità in futuro di aprire l'accesso al bot ad altri utenti admin (oltre a `ADMIN_ID`), che potrebbero così caricare schedine e correggere dati anche loro. Oggi il bot riconosce un solo `ADMIN_ID` hardcoded dall'env var — servirebbe passare a una lista di ID autorizzati (`ADMIN_IDS`) con lo stesso controllo ovunque viene fatto `update.effective_user.id != ADMIN_ID` (32 occorrenze in due categorie: 10 controlli d'accesso sui comandi, ~18 destinazioni `chat_id=ADMIN_ID` delle notifiche automatiche). Analizzato in dettaglio in Sessione 17 senza implementare. **Sarebbe "quasi pari funzionalità", non identica**: da decidere se il secondo admin ha accesso a `/setkey` e `/archiviastagione` (l'operazione che cancella dati, ora protetta da un codice a 6 cifre — vedi changelog Sessione 17) o solo alle operazioni quotidiane (caricamento schedine, correzioni). Servirebbe anche un log di "chi ha fatto cosa" sulle correzioni manuali (toccano punteggi e Cassa), e una convenzione tra i due admin per evitare azioni in contemporanea sulle operazioni delicate — non un lock nel codice, sproporzionato per due persone.
