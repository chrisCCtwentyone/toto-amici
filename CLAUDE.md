...
- **Chiamate a Gemini**: usa `chiama_gemini_con_retry()`. Non passa né da `richiedi_con_retry()` né da `num_retries` — è un terzo client ancora diverso (SDK `google-genai`). Riprova solo su 503/429/500, che sono picchi di carico temporanei; gli altri errori vanno rilanciati subito (Sessione 15).
...
- **Se l'utente segnala di nuovo una mail Render "exceeded memory limit"**: chiedi subito i log delle ultime ore (come fatto in Sessione 15) e controlla se il riavvio cade vicino a `task_aggiornamento_automatico` (schedulato alle 20:30 e 23:00 CEST) — in Sessione 15 la coincidenza era esatta. Quella funzione (`esegui_calcolo_risultati`) e anche `task_controlla_anomalie_partite` (ogni 2h) rileggono per intero `Giocate!A:I`/`A:G` a ogni chiamata, quindi il carico cresce con l'accumularsi delle giornate stagionali — vedi TODO in `PROJECT_LOG.md`.
